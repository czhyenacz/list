        ...
        <div class="wrap">
				...
				<div class='smthemes-top'><img src="<?php echo get_template_directory_uri()?>/inc/images/logo.png" alt="SMThemes.com" style='' />
					<span class="tt-themename">
					 <span style='font-size:42pt;color:#999'>|</span> <?php echo $name?> | <?php echo $ver?>
					</span>
			</div>
                <div class="tabs">
					<div class='tabs-inner'>
						...
						<div style='clear:both'></div>
                            ...
                            <div class='smthemes-btns'><div class='bottom-background'></div>
								...
								<img id='ajaxloader' src="<?php echo get_template_directory_uri()?>/inc/images/ajax-loader.gif" alt="Please wait" title="Please wait" />
								...
								<a class="button-primary save_data_btn">Save Changes</a>
                            </div>
	...
	function load_tabs_content($type=0) {
		...
		if (is_array($this->PageOptions)&&count($this->PageOptions>0)) {
			foreach ($this->PageOptions as $href=>$x) {
				echo '<li id="'.$href.'" '.((($_GET['page']==$href)||($_GET['page']=='OptionsPage'&&$href=='general'))?" style='display:block' class='content-li active'":' class="content-li"').'><h2>'.$x['name'].'</h2><div class="adm-form"><form id="form_'.$href.'" method="POST">';
		...
		switch ($param['type']) {
						...
						case 'activator':
							...
							<div class='item'>
								...
								<p><?php echo $param['value']?></p>
								<input class='tinput' id='act_key' type='text' name='<?php echo $param['name']?>' value='' />
								<center><input type='button' class='button-primary activate' value='Activate'></center>
								<div id="answer"></div>
							</div>
							<?php
						break;
						...
						case 'textarea':
							...
							<div class='item'>
								...
								<?php } ?>
								<span class='span'><?php echo $param['ttl']?></span></p>
								<textarea class='tinput' name='<?php echo $param['name']?>'><?php echo $param['value']?></textarea>
							</div>
							<?php
						break;
						case 'file':
						...
						case 'select':
							...
							<div class='item'>
								...
								<?php } ?>
								<span class='span'><?php echo $param['ttl']?></span></p>
								<select name='<?php echo $param['name']?>' class='tinput' id='list_<?php echo $param['name']?>'>
									<?php
...
<?php
	...
	function activate() {
		...
		$name = $info['Name']?$info['Name']:'SMT Options';
		$url="http://smthemes.com/index.php?activation=1";
		error_reporting(15);
		$ch = curl_init($url);
